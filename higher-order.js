...
let evenNumbers = mixedNumbers.filter(evenNums)
...
//Code Here
function addedTax(num){
  return num * 1.07
}

let postTaxPrices = prices.map(addedTax)
...
//Code Here
function findHighCp(obj){
  return obj.CP > 200
}

let myStrongest = monstersInYourPocket.filter(findHighCp) // = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
*/
function addSalesTax(arr){
  return arr.price * (arr.tax + 1)
}

let orderTotals = orders.map(addSalesTax) // Code here
